		...
		class SummarysetterView < View
			...
			def view_list_page(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('page.rhtml', 'summarysetter')).result(@api.ct_list_page(num).get_binding)
					ERB.new(self.load_admin_file).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
			...
			def view_list_details(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('details.rhtml', 'summarysetter')).result(_e.get_binding)
					ERB.new(self.load_admin_file).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
			...
			def view_add
				ERB.new(self.load_admin_file).result(ModuleTool::PagesetterList.new(self.load_static_file('add.html', 'summarysetter'), Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
			end
			...
			def view_edit(num = nil)
				begin
					...
					_e = @api.ct_list_details(num)
					return UtilsBase.inte_err_info(3001, "Page Not Exist") if _e.nil?
					cont = ERB.new(self.load_static_file('edit.rhtml', 'summarysetter')).result(_e.get_binding)
					ERB.new(self.load_admin_file).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
			...
			def view_delete(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('delete.rhtml', 'summarysetter')).result(binding)
					ERB.new(self.load_admin_file).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
