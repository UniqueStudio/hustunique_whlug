		...
		class EventsetterView < View
			...
			def default(num)
				begin
					...
					cont = ERB.new(self.load_static_file('list_events.rhtml', 'eventsetter')).result(@api.ct_list_pages(num).get_binding())
			...
			def view_add_event
				ERB.new(self.load_setter_html).result(ModuleTool::PagesetterList.new(self.load_static_file('add_event.html', 'eventsetter'), Disposition::COMMON_PAGE_EDIT_EVENT).get_binding)
			end

			def view_verify_add_event(*args); @api.ct_verify_add_event(*args); end
			...
			def view_list_event(num)
				begin
					...
					cont = ERB.new(self.load_static_file('list_detailed_event.rhtml', 'eventsetter')).result(_e.get_binding)
					ERB.new(self.load_setter_html).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_EVENT).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
			...
			def view_edit_event(num)
				begin
					...
					_e = @api.ct_list_events(num)
					return UtilsBase.inte_err_info(3001, "Page Not Exist") if _e.nil?
					cont = ERB.new(self.load_static_file('edit_event.rhtml', 'eventsetter')).result(_e.get_binding)
					ERB.new(self.load_setter_html).result(ModuleTool::PagesetterList.new(cont, Disposition::COMMON_PAGE_EDIT_EVENT).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
