		...
		class EventView < View
			...
			def view_list_page(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('page.rhtml', 'event')).result(@api.ct_list_page(num).get_binding)
					ERB.new(self.load_client_file).result(ModuleTool::PageList.new(cont, Disposition::COMMON_PAGE_EDIT_EVENT).get_binding)
				rescue
			...
			def view_list_details(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('details.rhtml', 'event')).result(_e.get_binding)
					ERB.new(self.load_client_file).result(ModuleTool::PageList.new(cont, Disposition::COMMON_PAGE_EDIT_EVENT).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
