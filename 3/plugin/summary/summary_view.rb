		...
		class SummaryView < View
			...
			def view_list_page(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('page.rhtml', 'summary')).result(@api.ct_list_page(num).get_binding)
					ERB.new(self.load_client_file).result(ModuleTool::PageList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
			...
			def view_list_details(num = nil)
				begin
					...
					cont = ERB.new(self.load_static_file('details.rhtml', 'summary')).result(_e.get_binding)
					ERB.new(self.load_client_file).result(ModuleTool::PageList.new(cont, Disposition::COMMON_PAGE_EDIT_SUMMARY).get_binding)
				rescue
					UtilsBase.dev_mode? and raise
					UtilsBase.inte_bigerr_info
