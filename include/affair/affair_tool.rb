...
module Ash
  module ModuleTool
    ...
    AffairDetBriefs = Struct.new(:time, :title, :nid, :content, :hits)
    ...
    class AffairTool
      ...
      def find_det_briefs_by_page(num)
        ...
        return if res.empty?
        final = []
        res.map {|l| final << AffairDetBriefs.new(UtilsBase.format_det_time(l['time']), l['title'], l['nid'].to_i, UtilsBase.html_gsub(l['content']), l['hits'].to_i)}
        final
      end
